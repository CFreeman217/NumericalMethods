...
def me306_final_1():
    ...
    def analytical(in_val):
        epow = 10 * (0.15)**(0.5)
        lside = 150 - 240*math.exp(epow)
        c_2 = lside / (math.exp(-epow) - math.exp(epow))
        c_1 = 240 - c_2 
        return (c_1 * math.exp(in_val*(0.15**0.5)) + c_2 * math.exp(-in_val*(0.15**0.5)))
    X1 = np.arange(0, 2, 1)
    Y1 = [analytical(i) for i in X1]
    x_startstop = [0,10]
    needed_val = 150
    ...
    plt.plot(X2, Y2[:,0], label='Shooting Method')
    plt.xlabel('X - Values')
    plt.ylabel('Y - Values')
